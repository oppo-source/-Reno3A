...
#ifdef VENDOR_EDIT
/* wangdengwen@oppo.com   2020-04-20  for charge full at SDP type */
		rc = smblib_get_prop_usb_present(chg, val);
#endif
...
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
		if(g_oppo_chip){
			do_gettimeofday(&(g_oppo_chip->stChargerDetect.stVbusOnTime));
			if(g_oppo_chip->stChargerDetect.isForceType){
...
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
		if(g_oppo_chip){
			do_gettimeofday(&(g_oppo_chip->stChargerDetect.stVbusOffTime));
			smblib_err(chg, "WDWCHG VBUS OFF time=%d\n",(g_oppo_chip->stChargerDetect.stVbusOffTime.tv_sec - g_oppo_chip->stChargerDetect.stVbusOnTime.tv_sec));
			if((g_oppo_chip->stChargerDetect.stVbusOffTime.tv_sec - g_oppo_chip->stChargerDetect.stVbusOnTime.tv_sec) > 3){
				smblib_err(chg, "WDWCHG VBUS OFF reset\n");
				g_oppo_chip->stChargerDetect.iQcDetectCount   = 0;
				g_oppo_chip->stChargerDetect.iDcpDetectCount  = 0;
				g_oppo_chip->stChargerDetect.iUsbPlugIrqCount = 0;
			}
			if(g_oppo_chip->stChargerDetect.isResetType){
				smblib_err(chg, "WDWCHG RERUN HVDCP&APSD rc=%d, forcetype=%d\n", rc , g_oppo_chip->stChargerDetect.isForceType);
				smblib_hvdcp_detect_enable(chg, true);
			}
		}
#endif
...
static void typec_src_insertion(struct smb_charger *chg)
{
	...
	if (!chg->ok_to_pd && g_oppo_chip && !g_oppo_chip->stChargerDetect.isForceType)
		smblib_hvdcp_detect_enable(chg, true);
}
