...
#ifdef VENDOR_EDIT
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
...
#ifdef VENDOR_EDIT
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
	chip->stChargerDetect.isResetType = false;
	chip->stChargerDetect.isForceType = false;
	chip->stChargerDetect.iDcpDetectCount = 0;
	chip->stChargerDetect.isRecoverUsbIn = false;
	chip->stChargerDetect.iQcDetectCount = 0;
	chip->stChargerDetect.iUsbPlugIrqCount = 0;
#endif
	chip->limits.qc_input_volt_charger_v = VBUS_LIMIT_9V;
#ifdef CONFIG_OPPO_CHARGER_MTK
	chip->usb_online = false;
...
#ifdef VENDOR_EDIT
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
	static bool isChargerTypeCheck = false;
#endif
#ifdef CONFIG_OPPO_CHARGER_MTK
...
#ifdef VENDOR_EDIT
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
	if(!isChargerTypeCheck && chip->stChargerDetect.iDcpDetectCount > 2){
		if(!chip->stChargerDetect.isTimeCountStart){  //start timecount
			do_gettimeofday(&(chip->stChargerDetect.stTimeStart));
			chip->stChargerDetect.isTimeCountStart = true;
		}
		do_gettimeofday(&(chip->stChargerDetect.stTimeEnd));
		if(!chip->stChargerDetect.isForceType && ((chip->stChargerDetect.stTimeEnd.tv_sec - chip->stChargerDetect.stTimeStart.tv_sec) >= 30)){
			chip->stChargerDetect.isForceType      = false;
			chip->stChargerDetect.iQcDetectCount   = 0;
			chip->stChargerDetect.iDcpDetectCount  = 0;
			chip->stChargerDetect.iUsbPlugIrqCount = 0;
			...
			isChargerTypeCheck = true;
		}
		else{
			if((chip->stChargerDetect.iDcpDetectCount > 2 && chip->stChargerDetect.iQcDetectCount > 2) || chip->stChargerDetect.iUsbPlugIrqCount > 4){
				chip->stChargerDetect.isForceType    = true;
				chip->stChargerDetect.isResetType    = false;
				chip->stChargerDetect.isRecoverUsbIn = false;
				isChargerTypeCheck = true;
				charger_xlog_printk(CHG_LOG_CRTI, "Charger_not_normal--isReset =%d\n", chip->stChargerDetect.isResetType);
			}
		}
	}
#endif
...
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
		if( chip->stChargerDetect.isForceType ){
			if(!chip->stChargerDetect.isResetType)
				chip->stChargerDetect.isResetType      = true;
			else{
				chip->stChargerDetect.isForceType      = false;
				chip->stChargerDetect.iDcpDetectCount  = 0;
				chip->stChargerDetect.iQcDetectCount   = 0;
...
#ifdef VENDOR_EDIT
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
	if( chip->stChargerDetect.isRecoverUsbIn ){
		oppo_chg_set_input_current_limit(chip);
		chip->stChargerDetect.isRecoverUsbIn = false;
	}
#endif
...
#ifdef VENDOR_EDIT
/*Wenboyu@BSP.CHG.Basic Solve after SDP full and charger suspend, flash and torch didn't work*/
		rc = smblib_write(&chip->pmic_spmi.smb5_chip->chg, 0xA663, 1);
		if(rc < 0){
			pr_err("Couldn't write to 0xA663 rc=%d\n",rc);
			return rc;
		}
#endif
		ret.intval = 1;
