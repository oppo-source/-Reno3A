...
struct charger_type_detect{
	int iDcpDetectCount;          //DCP detected count
	int iQcDetectCount;           //HVDCP2 detected count
	int iUsbPlugIrqCount;         //record irq count
	int iCheckStartCount;         //DCP detected count
	int iCheckTimeLast;           //default set 30 second to check
	bool isRecoverUsbIn;          //recover usbin after adapter is forced to DCP
	bool isForceType;             //true:force adapter to DCP
	bool isResetType;             //check if usb plugout after force adapter to DCP
	bool isTimeCountStart;        //check start flag
	struct timeval stTimeStart;   //check start time
	struct timeval stTimeEnd;     //check end time
	struct timeval stVbusOnTime;  //vbus on time, adatper in
	struct timeval stVbusOffTime; //vbus off time, adapter out
};
#endif
...
#ifdef VENDOR_EDIT
/* wangdengwen@oppo.com  changed PD adapter compatible at 6125Q */
	struct charger_type_detect stChargerDetect;
#endif
#ifdef	 CONFIG_OPPO_CHARGER_MTK
	bool usb_online;
#endif
